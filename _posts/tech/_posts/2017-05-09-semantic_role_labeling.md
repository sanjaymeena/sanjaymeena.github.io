...
SRL is useful as an intermediate step in a wide range of natural language processing (NLP) tasks, such as information extraction, automatic document categorization, question anwering etc. 
...
Syntanctic information is considered to play an essential role in solving this problem. The location of an argument on syntactic tree provides an intermediate tag for improving the performance. 
...
Here we will discuss an end-to-end system using bi-directional long short-term memory (DB-LSTM) which was presented in : 
...
People often met with two diffuculties with RNN :
1) Information of the current word strongly depends on distant words, rather than its neighbourhood. 
2) Gradient parameters may explode or vanish especially in processing long sequences (Bengio et al. 1994) . Long short-term memory (LSTM) (Hochreiter and Schmidhuber, 1997) was propsed to address the above difficulties. 
...
Word embeddings
Pre-trained word embedings are used to address the data sparsity issuee. 

Training Algorithm 
Schoastic gradiend descent (SGD) algoirhtm is used as the training technique for the whole pipeline. 
